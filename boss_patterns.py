# boss_patterns.py
from coordinate_mapping import PATTERN_X_MIN, PATTERN_X_MAX, PATTERN_Y_MIN, PATTERN_Y_MAX
...
def normal_scratch3(boss_cell: tuple[int, int]) -> list[tuple[int, int]]:
    ...
    for dx in range(-1, 2):
        for dy in range(-1, 2):
            ...
            if PATTERN_X_MIN <= cx <= PATTERN_X_MAX and PATTERN_Y_MIN <= cy <= PATTERN_Y_MAX:
                result.append((cx, cy))
    return result
...
def forward_stripe3_from_cell(boss_cell: tuple[int, int], facing: tuple[int, int]) -> list[tuple[int, int]]:
    ...
    while PATTERN_X_MIN <= curr[0] <= PATTERN_X_MAX and PATTERN_Y_MIN <= curr[1] <= PATTERN_Y_MAX:
        for offset in range(-1, 2):
...
def horizontal_stripe3(y: int) -> list[tuple[int, int]]:
    result = []
    for x in range(PATTERN_X_MIN, PATTERN_X_MAX + 1):
        for dy in range(-1, 2):
...
def vertical_stripe3(x: int) -> list[tuple[int, int]]:
    result = []
    for y in range(PATTERN_Y_MIN, PATTERN_Y_MAX + 1):
        for dx in range(-1, 2):
...
def left_edge2() -> list[tuple[int, int]]:
    ...
    for y in range(PATTERN_Y_MIN, PATTERN_Y_MAX + 1):
        for dx in range(0, 2):
            result.append((PATTERN_X_MIN + dx, y))
    return result
...
def right_edge2() -> list[tuple[int, int]]:
    ...
    for y in range(PATTERN_Y_MIN, PATTERN_Y_MAX + 1):
        for dx in range(0, 2):
            result.append((PATTERN_X_MAX - dx, y))
    return result
...
def top_edge2() -> list[tuple[int, int]]:
    ...
    for x in range(PATTERN_X_MIN, PATTERN_X_MAX + 1):
        for dy in range(0, 2):
            result.append((x, PATTERN_Y_MAX - dy))
    return result
...
def bottom_edge2() -> list[tuple[int, int]]:
    ...
    for x in range(PATTERN_X_MIN, PATTERN_X_MAX + 1):
        for dy in range(0, 2):
            result.append((x, PATTERN_Y_MIN + dy))
    return result
...
def directional_edge_cell(start: tuple[int, int], direction: tuple[int, int]) -> tuple[int, int]:
    ...
    while True:
        ...
        if not (PATTERN_X_MIN <= nxt[0] <= PATTERN_X_MAX and PATTERN_Y_MIN <= nxt[1] <= PATTERN_Y_MAX):
            break
        curr = nxt
    return curr
...
def get_centered_dash_start(damage_cells: list[tuple[int, int]], dash_dir: tuple[int, int]) -> tuple[int, int]:
    ...
    center_x = max(PATTERN_X_MIN, min(PATTERN_X_MAX, round(avg_x)))
    center_y = max(PATTERN_Y_MIN, min(PATTERN_Y_MAX, round(avg_y)))
    
    if dash_dir[0] > 0:
        return (PATTERN_X_MIN, center_y)
    if dash_dir[0] < 0:
        return (PATTERN_X_MAX, center_y)
    if dash_dir[1] > 0:
        return (center_x, PATTERN_Y_MIN)
    if dash_dir[1] < 0:
        return (center_x, PATTERN_Y_MAX)
        
    return (center_x, center_y)

def get_centered_dash_end(start: tuple[int, int], dash_dir: tuple[int, int]) -> tuple[int, int]:
    if dash_dir[0] > 0:
        return (PATTERN_X_MAX, start[1])
    if dash_dir[0] < 0:
        return (PATTERN_X_MIN, start[1])
    if dash_dir[1] > 0:
        return (start[0], PATTERN_Y_MAX)
    if dash_dir[1] < 0:
        return (start[0], PATTERN_Y_MIN)
