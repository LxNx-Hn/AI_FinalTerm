param(
    [string]$RunId  = "BossPPO_TrainStable_20k_v1",
    [switch]$Force,
    [switch]$Resume
)
...
if ($portCheck) { Write-Host "[20k_train] Port 5004 in use -- abort"; exit 1 }
Write-Host "[20k_train] run-id=$RunId"
Write-Host "[20k_train] max_steps=20000  summary_freq=1000  timeout-wait=300"
Write-Host "[20k_train] Stable settings: LoadSceneAsync  TakeActionsBetweenDecisions=false  Discrete[6]  Obs=193"
Write-Host "[20k_train] Reward: boss_dmg=0.10 kill=5.0 miss=-0.05 cooldown=-0.02 hit=-2.0 death=-8.0 warn=-0.05 dmg=-0.20 wall=-0.05 step=-0.001 approach=0"
...
Write-Host "[20k_train] trainer exited: $exitCode"
exit $exitCode
