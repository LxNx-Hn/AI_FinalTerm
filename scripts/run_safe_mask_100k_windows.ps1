param(
    [string]$RunId       = "BossPPO_SafeActionMask_100k_v1",
    [string]$InitFrom    = "BossPPO_SafeActionMask_50k_v1",
    [switch]$Force,
    [switch]$Resume,
    [switch]$NoInitFrom
)
...
$Config        = Join-Path $ProjectRoot "ml-agents-config\boss_ppo_safe_mask_100k_v1.yaml"
...
Write-Host "[run] max_steps=100000  EXE=$EnvExe"
