param(
    [string]$RunId = "BossPPO_SafeOppMoveTrace_Diag_v1",
    [switch]$Force
)
...
$Config        = Join-Path $ProjectRoot "ml-agents-config\boss_ppo_safe_opp_move_trace_diag_v1.yaml"
...
if ($portCheck) { Write-Host "[safe_opp_move_trace_diag] Port 5004 in use; stop trainer first."; exit 1 }
...
Write-Host "[safe_opp_move_trace_diag] run-id=$RunId force=$Force"
Write-Host "[safe_opp_move_trace_diag] max_steps=10000 timeout=210s EXE=$EnvExe"
Write-Host "[safe_opp_move_trace_diag] trace-only diagnostic: reward values unchanged"
Write-Host "[safe_opp_move_trace_diag] fresh diagnostic only: no --resume and no --initialize-from"
...
Write-Host "[safe_opp_move_trace_diag] trainer exited: $exitCode"
exit $exitCode
