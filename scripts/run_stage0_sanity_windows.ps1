param(
    [string]$RunId  = "BossPPO_ActionWrapperSanity_v1",
    [switch]$Force,
    [switch]$Resume
)
...
$Config        = Join-Path $ProjectRoot "ml-agents-config\boss_player_action_sanity_stage0.yaml"
...
if ($portCheck) { Write-Host "[stage0] Port 5004 in use"; exit 1 }
Write-Host "[stage0] run-id=$RunId  max_steps=2000  survival_stage=1.0 (ATTACK masked)"
...
Write-Host "[stage0] trainer exited: $exitCode"
exit $exitCode
